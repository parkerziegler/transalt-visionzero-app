window.onload = function () {
  ...
  L.tileLayer(
    "https://api.mapbox.com/styles/v1/{id}/tiles/{z}/{x}/{y}?access_token={accessToken}",
    {
      ...
      id: "mapbox/dark-v10",
      accessToken:
        "pk.eyJ1IjoicHppZWdsZXIiLCJhIjoiY2ltMHo3OGRxMDh0MXR5a3JrdHNqaGQ0bSJ9.KAFBMeyysBLz4Ty-ltXVQQ",
    }
  ).addTo(map);
  ...
  cartodb
    .createLayer(map, layerSource, { legends: true, https: true })
    .addTo(map)
    .done(function (layer) {
  ...
  cartodb
    .createLayer(map, torqueSource, { legends: true })
    .addTo(map)
    .done(function (layer) {
  ...
  var ZoomActions = {
    reset: function () {
      map.setView([40.73885, -73.889837], 12);
    },
    bronx: function () {
    ...
    brooklyn: function () {
      var brooklynXY = [40.680862, -73.96541];
  ...
  geojson.eachLayer(function (layer) {
    layer.bindLabel(
      "<u>City Council District</u><p>" +
        "<u>" +
        layer.feature.properties.coun_dist +
        "</u></p><br> Crashes per Sq. Mi. <p>" +
        layer.feature.properties.crash_area.toFixed(1) +
        "</p>" +
        "<br> Population Density <p>" +
        layer.feature.properties.pop_density.toLocaleString("en", {
  ...
  svgBar
    ...
    .attr("font-family", "HelveticaNeue, Helvetica, sans-serif")
    .attr("font-size", "8px")
    .attr("fill", "white")
  ...
  svgBar
    ...
    .attr(
      "transform",
      "translate(" + margin.left + "," + (margin.top / 2 + h) + ")"
    )
    .call(xAxisBar);
  ...
  var D3BarActions = {
    bronx: function () {
      ...
      svgBar.select(".x.axis").transition().duration(1500).call(xAxisBar);

      svgBar.select(".y.axis").transition().duration(1500).call(yAxisBar);

      // select the rects and append the data
      var bars = svgBar.selectAll("rect").data(datasetBar);
      ...
      // select the labels
      var labels = svgBar.selectAll(".chart-label").data(datasetBar);
      ...
      // exit
      labels.exit().transition().duration(1000).attr("opacity", 0).remove();
